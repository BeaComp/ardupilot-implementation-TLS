 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
bool AP_Frsky_SPort_Passthrough::is_passthrough_byte(const uint8_t var_byte) const
{
#if HAL_WITH_FRSKY_TELEM_BIDIRECTIONAL
    if( var_byte == _SPort_bidir.downlink1_sensor_id || var_byte == _SPort_bidir.downlink2_sensor_id ) {
        return true;
    }
#endif
    return var_byte == downlink_sensor_id;
}
